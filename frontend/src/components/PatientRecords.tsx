...
export function PatientRecords({ userRole }: PatientRecordsProps) {
  ...
  const mapDoc = (doc: ApiPatient): PatientRecord => ({
    id: doc._id || doc.id || "",
    name: doc.name || "—",
    age: doc.dob
    ...
    currentMedications: Array.isArray(doc.medications)
      ? doc.medications.map((m: ApiMedication) =>
          `${m.name} ${m.dose || ""}`.trim(),
        )
      : [],
    diagnoses: [],
  ...
  const toApiPayload = (p: PatientRecord) => {
    const now = new Date();
    const dob =
      p.age > 0 ? new Date(now.getFullYear() - p.age, 0, 1) : undefined;
    return {
      name: p.name,
      dob,
