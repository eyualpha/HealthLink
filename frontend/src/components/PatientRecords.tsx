    ...
    treatments: [
      {
        ...
        notes: "Increased Metformin dosage",
      },
    ],
    immunizations: [
...
export function PatientRecords({ userRole }: PatientRecordsProps) {
  ...
  const [editablePatient, setEditablePatient] = useState<PatientRecord | null>(
    null,
  );
  ...
  type ApiMedication = { name?: string; dose?: string };
  type ApiDiagnosis = { date?: string; condition?: string; name?: string; doctor?: string; provider?: string };
  type ApiTreatment = { date?: string; treatment?: string; name?: string; notes?: string };
  type ApiImmunization = { vaccine?: string; name?: string; date?: string };
  type ApiLabResult = { test?: string; name?: string; date?: string; result?: string; value?: string; status?: string };
  type ApiPatient = {
    _id?: string;
    id?: string;
    name?: string;
    ...
    immunizations?: ApiImmunization[];
    labResults?: ApiLabResult[];
    updatedAt?: string;
  };

  const mapDoc = useCallback((doc: ApiPatient): PatientRecord => ({
    id: doc._id || doc.id || "",
    name: doc.name || "—",
    age: doc.dob
    ...
    currentMedications: Array.isArray(doc.medications)
      ? doc.medications.map((m: ApiMedication) => `${m.name} ${m.dose || ""}`.trim())
      : [],
    diagnoses: Array.isArray(doc.diagnoses)
      ? doc.diagnoses.map((d: ApiDiagnosis) => ({
          date: d.date || "",
    ...
    treatments: Array.isArray(doc.treatments)
      ? doc.treatments.map((t: ApiTreatment) => ({
          date: t.date || "",
    ...
    immunizations: Array.isArray(doc.immunizations)
      ? doc.immunizations.map((i: ApiImmunization) => ({
          vaccine: i.vaccine || i.name || "-",
          date: i.date || "",
        }))
      : [],
    labResults: Array.isArray(doc.labResults)
      ? doc.labResults.map((l: ApiLabResult) => ({
          test: l.test || l.name || "-",
          date: l.date || "",
  ...
  const toApiPayload = (p: PatientRecord) => {
    const now = new Date();
    const dob = p.age > 0 ? new Date(now.getFullYear() - p.age, 0, 1).toISOString().split("T")[0] : undefined;
    return {
      name: p.name,
      dob,
      ...
      medications: (p.currentMedications || []).map((m) => ({ name: m })),
      medicalHistory: p.medicalHistory,
      identifiers: p.id ? [{ system: "local", value: p.id }] : [],
    };
  };

  const handleSave = async (p: PatientRecord, isCreate?: boolean) => {
    setSaveError(null);
    try {
      if (isCreate) {
        const created = await createPatient(toApiPayload(p));
        setPatients((prev) => [mapDoc(created), ...prev]);
      } else {
        const updated = await updatePatient(p.id, toApiPayload(p));
        setPatients((prev) =>
          prev.map((x) => (x.id === p.id ? mapDoc(updated) : x)),
        );
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      {saveError && (
        <div className="bg-red-50 text-red-700 px-4 py-3 text-sm rounded-lg">
          {saveError}
        </div>
      )}
